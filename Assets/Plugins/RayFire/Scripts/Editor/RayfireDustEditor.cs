...
namespace RayFireEditor
{
    ...
    public class RayfireDustEditor : Editor
    {
        ...
        void GUI_Emit()
        {
            RFUI.CaptionBox (TextDst.gui_cap_event);
            RFUI.PropertyField (sp_main_dml, TextDst.gui_main_dml);
            RFUI.PropertyField (sp_main_act, TextDst.gui_main_act);
            RFUI.PropertyField (sp_main_imp, TextDst.gui_main_imp);
            if (sp_main_dml.boolValue == false && sp_main_act.boolValue == false && sp_main_imp.boolValue == false)
                RFUI.HelpBox (TextDst.hlp_select, MessageType.Warning, true);
        }
        ...
        void GUI_Main()
        {
            RFUI.CaptionBox (TextDst.gui_cap_dust);
            RFUI.Slider (sp_main_op, opacity_min, opacity_max, TextDst.gui_main_op);
            RFUI.PropertyField (sp_main_mat, TextDst.gui_main_mat);
            RFUI.Foldout (ref fld_mat, TextKeys.dst_fld_mat, TextDst.gui_main_mat_list.text);
            if (fld_mat == true)
            {
                RFUI.Space ();
        ...
        void UI_Properties()
        {
            RFUI.CaptionBox (TextDst.gui_cap_props);
            GUI_Pool();
            GUI_Emission();
            GUI_Dynamic();
            GUI_Noise();
        ...
        void GUI_Pool()
        {
            RFUI.Foldout (ref fld_pol, TextKeys.dst_fld_pol, TextDst.gui_fld_pol.text);
            if (fld_pol == true)
            {
        ...
        void GUI_Emission()
        {
            RFUI.Foldout (ref fld_emt, TextKeys.dst_fld_emt, TextDst.gui_fld_emt.text);
            if (fld_emt == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_burst);
                RFUI.PropertyField (sp_ems_tp, TextDst.gui_ems_tp);
                if (dust.emission.burstType != RFParticles.BurstType.None)
                {
                    RFUI.IntSlider (sp_ems_am,  ems_am_min,  ems_am_max,  TextDst.gui_ems_am);
                    RFUI.IntSlider (sp_ems_var, ems_var_min, ems_var_max, TextDst.gui_ems_var);
                }
                RFUI.Caption (TextDst.gui_cap_dist);
                RFUI.Slider (sp_ems_rate, ems_rate_min, ems_rate_max, TextDst.gui_ems_rate);
                RFUI.Slider (sp_ems_dur,  ems_dur_min,  ems_dur_max,  TextDst.gui_ems_dur);
                RFUI.Caption (TextDst.gui_cap_life);
                RFUI.Slider (sp_ems_life_min, ems_life_min, ems_life_max, TextDst.gui_ems_life_min);
                RFUI.Slider (sp_ems_life_max, ems_life_min, ems_life_max, TextDst.gui_ems_life_max);
                RFUI.Caption (TextDst.gui_cap_size);
                RFUI.Slider (sp_ems_size_min, ems_size_min_min, ems_size_min_max, TextDst.gui_ems_size_min);
                RFUI.Slider (sp_ems_size_max, ems_size_max_min, ems_size_max_max, TextDst.gui_ems_size_max);
                RFUI.Caption (TextDst.gui_cap_mat);
                RFUI.PropertyField (sp_ems_mat, TextDst.gui_ems_mat);
                EditorGUI.indentLevel--;
            }
        }
        ...
        void GUI_Dynamic()
        {
            RFUI.Foldout (ref fld_dyn, TextKeys.dst_fld_dyn, TextDst.gui_fld_dyn.text);
            if (fld_dyn == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_speed);
                RFUI.Slider (sp_dyn_speed_min, dyn_speed_min, dyn_speed_max, TextDst.gui_dyn_speed_min);
                RFUI.Slider (sp_dyn_speed_max, dyn_speed_min, dyn_speed_max, TextDst.gui_dyn_speed_max);
                RFUI.Caption (TextDst.gui_cap_grav);
                RFUI.Slider (sp_dyn_grav_min, dyn_grav_min, dyn_grav_max, TextDst.gui_dyn_grav_min);
                RFUI.Slider (sp_dyn_grav_max, dyn_grav_min, dyn_grav_max, TextDst.gui_dyn_grav_max);
                RFUI.Caption (TextDst.gui_cap_rot);
                RFUI.Slider (sp_dyn_rot, dyn_rot_min, dyn_rot_max, TextDst.gui_dyn_rot);
                EditorGUI.indentLevel--;
            }
        }
        ...
        void GUI_Noise()
        {
            RFUI.Foldout (ref fld_nse, TextKeys.deb_fld_nse, TextDst.gui_fld_nse.text);
            if (fld_nse == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_main);
                RFUI.PropertyField (sp_nse_en, TextDst.gui_nse_en);
                if (dust.noise.enabled == true)
                {
                    RFUI.PropertyField (sp_nse_qual, TextDst.gui_nse_qual);
                    RFUI.Caption (TextDst.gui_cap_str);
                    RFUI.Slider (sp_nse_str_min, nse_str_min, nse_str_max, TextDst.gui_nse_str_min);
                    RFUI.Slider (sp_nse_str_max, nse_str_min, nse_str_max, TextDst.gui_nse_str_max);
                    RFUI.Caption (TextDst.gui_cap_other);
                    RFUI.Slider (sp_nse_freq,   nse_freq_min,   nse_freq_max,   TextDst.gui_nse_freq);
                    RFUI.Slider (sp_nse_scroll, nse_scroll_min, nse_scroll_max, TextDst.gui_nse_scroll);
                    RFUI.PropertyField (sp_nse_damp, TextDst.gui_nse_damp);
                }
                EditorGUI.indentLevel--;
            }
        }
        ...
        void GUI_Collision()
        {
            RFUI.Foldout (ref fld_col, TextKeys.dst_fld_col, TextDst.gui_fld_col.text);
            if (fld_col == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_common);
                RFUI.MaskField(sp_col_mask, TextDst.gui_col_mask);
                RFUI.PropertyField (sp_col_qual, TextDst.gui_col_qual);
                RFUI.Slider (sp_col_rad, col_rad_min, col_rad_max, TextDst.gui_col_rad);
                EditorGUI.indentLevel--;
            }
        }
        ...
        {
            RFUI.Foldout (ref fld_lim, TextKeys.dst_fld_lim, TextDst.gui_fld_lim.text);
            if (fld_lim == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_prt);
                RFUI.IntSlider (sp_lim_prt_min, lim_prt_min, lim_prt_max, TextDst.gui_lim_prt_min);
                RFUI.IntSlider (sp_lim_prt_max, lim_prt_min, lim_prt_max, TextDst.gui_lim_prt_max);
                RFUI.PropertyField (sp_lim_prt_vis, TextDst.gui_lim_prt_vis);
                RFUI.Caption (TextDst.gui_cap_frags);
                RFUI.IntSlider (sp_lim_perc, lim_perc_min, lim_perc_max, TextDst.gui_lim_perc);
                RFUI.Slider (sp_lim_size, lim_size_min, lim_size_max, TextDst.gui_lim_size);
                EditorGUI.indentLevel--;
            }
        }
        ...
        void GUI_Rendering()
        {
            RFUI.Foldout (ref fld_rnd, TextKeys.dst_fld_rnd, TextDst.gui_fld_ren.text);
            if (fld_rnd == true)
            {
                EditorGUI.indentLevel++;
                RFUI.Caption (TextDst.gui_cap_shad);
                RFUI.PropertyField (sp_ren_cast, TextDst.gui_ren_cast);
                RFUI.PropertyField (sp_ren_rec,  TextDst.gui_ren_rec);
                RFUI.Caption (TextDst.gui_cap_other);
                RFUI.PropertyField (sp_ren_prob, TextDst.gui_ren_prob);
                RFUI.PropertyField (sp_ren_vect, TextDst.gui_ren_vect);
                RFUI.PropertyField (sp_ren_t,    TextDst.gui_ren_t);
                if (dust.rendering.t == true)
                    RFUI.TagField (sp_ren_tag, TextDst.gui_ren_tag);
                RFUI.PropertyField (sp_ren_l, TextDst.gui_ren_l);
                if (dust.rendering.l == true)
                    RFUI.LayerField (sp_ren_lay, TextDbr.gui_ren_lay);
                EditorGUI.indentLevel--;
            }
        }
