...
{
    [SelectionBase]
    [AddComponentMenu (RFLog.dst_path)]
    [HelpURL (RFLog.dst_link)]
    public class RayfireDust : MonoBehaviour
    {
        ...
        void SetDustMaterials()
        {
            ...
            {
                RayfireMan.Log (RFLog.dst_dbgn + name + RFLog.dst_noMat, gameObject);
                initialized = false;
                return;
            }
            ...
            if ((int)emission.burstType == 1)
                RayfireMan.Log (RFLog.dst_dbgn + name + RFLog.dst_noBurs, gameObject);
